...
- **Lockstep Dev/Prod Deployments & On-Demand Qualification**: The development stack (`dev-router-pod`) is **disabled and down by default** (its Quadlets omit `WantedBy=default.target`). It serves as an on-demand qualification step for end-to-end testing prior to release, and as a standby backup target in HAProxy (`server dev_* check backup`) if production is offline. Start it on demand with `DEV_ENV_FILE=.env.dev ./start-stack.sh` (or `--replace` / `--pull`), validate, and tear it down afterward with `DEV_ENV_FILE=.env.dev ./start-stack.sh --stop`. Manifests and image tags must always be kept in lockstep with production.
