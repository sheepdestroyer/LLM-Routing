...
Fallback Tiers (same conversation, different model):
  ...
  Tier 2: claude-opus-3-5@default     (premium tier)
...
AGY_FALLBACK_TIERS = [
    ...
    {"model_name": "claude-opus-3.5",   "env_override": "claude-opus-3-5@default"},      # Tier 2
]
...
async def try_agy_proxy(request: AgyProxyRequest) -> Optional[dict]:
    ...
    # Advanced: full 2-tier chain (gemini-3.5-flash → claude-opus-3.5)
    if target_tier == "agent-reasoning-core":
        agy_tiers = [
            {"model_name": "gemini-3.5-flash", "env_override": ""},  # low thinking default
        ]
    else:
        agy_tiers = AGY_FALLBACK_TIERS  # full chain: gemini-3.5-flash → claude-opus-3.5
    ...
    try:
        ...
        for tier_idx, tier in enumerate(agy_tiers[start_tier_index:]):
            ...
            # Tier 1 (idx 1): claude-opus-3.5  → vendor_breaker
