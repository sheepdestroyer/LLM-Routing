...
class PerModelBreaker:
    ...
    def is_allowed(self) -> bool:
        ...
        if now >= self.cooldown_until:
            if not self.probe_granted:
                ...
                logger.info(
                    f"Circuit breaker [{self.name}]: Tier {self.tier} cooldown expired. Granting 1 probe request."
                )
                return True
            return False

        remaining = self.cooldown_until - now
        logger.debug(f"Circuit breaker [{self.name}]: blocked (tier {self.tier}, {remaining:.0f}s remaining)")
        return False
    ...
    def record_success(self):
        ...
        if self.tier > 0:
            logger.info(
                f"Circuit breaker [{self.name}]: probe succeeded — resetting from Tier {self.tier} to Tier 0 (open)"
            )
        self.tier = 0
...
class DualCircuitBreaker:
    ...
    async def sync_from_valkey(self, redis_client) -> None:
        ...
        await asyncio.gather(self.google.sync_from_valkey(redis_client), self.vendor.sync_from_valkey(redis_client))

    async def save_to_valkey(self, redis_client) -> None:
        """Persist both sub-breakers to Valkey."""
        import asyncio

        await asyncio.gather(self.google.save_to_valkey(redis_client), self.vendor.save_to_valkey(redis_client))
