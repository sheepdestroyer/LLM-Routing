...
def test_build_model_suites(sync_engine):
    ...
    assert any(m["model_name"] == "local-qwen" for m in locallama)
    assert any(m["model_name"] == "locallama-qwen-hass" for m in locallama)
    assert any(m["model_name"] == "local-qwen-hass" for m in locallama)
    assert any(m["model_name"] == "locallama-qwen-routing" for m in locallama)
    assert any(m["model_name"] == "local-qwen-routing" for m in locallama)
    ...
    assert any(m["model_name"] == "whisper-1" for m in locallama)
    ...
    assert any(m["model_name"] == "llm-routing-agy" for m in agy)
    assert any(m["model_name"] == "llm-routing-agy-sse" for m in agy)
    assert any(m["model_name"] == "agy-sse" for m in agy)
