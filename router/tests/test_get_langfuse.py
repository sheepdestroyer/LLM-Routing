...
def test_get_langfuse_success():
    ...
    with patch.dict("sys.modules", {"langfuse": mock_langfuse_module}), patch("router.main.LANGFUSE_HOST", "host"):
        with patch.dict(
            os.environ, {"LANGFUSE_PUBLIC_KEY": "pk", "LANGFUSE_SECRET_KEY": "sk", "LANGFUSE_HOST": "host"}
        ):
            client = get_langfuse()
            assert client is mock_langfuse_instance
            mock_langfuse_module.Langfuse.assert_called_once_with(
                public_key="pk", secret_key="sk", host="host", release="llm-triage-router-v1"
            )
