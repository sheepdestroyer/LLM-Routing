...
async def _read_annotations_async(path) -> dict:
    ...
    if cache_entry is None or current_mtime != cache_entry["mtime"]:
        async with aiofiles.open(path, "rb") as f:
            # Performance optimization (Bolt): Cache raw bytes instead of parsed dicts.
            # orjson.loads(bytes) is significantly faster than copy.deepcopy(dict) for returning safe copies.
            content = await f.read()
