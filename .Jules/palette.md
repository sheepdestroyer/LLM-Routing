...
## 2024-08-25 - Avoid anchor tags for JavaScript actions
**Learning:** Using `<a href="#">` for purely JavaScript-driven actions (like clearing form data or annotations) forces unintended page scrolls and breaks semantics.
**Action:** Replace `<a href="#">` tags used for JS actions with semantically correct `<button type="button">` tags. Use inline CSS (like `background: transparent; border: none; padding: 0; font-family: inherit; font-size: inherit; cursor: pointer; text-decoration: underline;`) to maintain the visual appearance of a link while preserving correct interaction semantics and accessibility without side effects.
