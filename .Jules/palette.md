...
## 2024-05-19 - Replace interactive anchors with semantic buttons
**Learning:** Found `<a href="#" onclick="...">` used for interactive actions (like clearing annotations). Clicking these elements causes the page to inadvertently scroll to the top due to the default behavior of the `#` fragment, disrupting the user experience.
**Action:** Replaced the anchor element with a native `<button type="button">`, styled with CSS resets (`background: transparent; border: none; cursor: pointer; text-decoration: underline; padding: 0; display: inline-block;`) to maintain the visual appearance of a link while providing robust semantics, keyboard accessibility, and preventing the default scroll jump issue. Added focus visible styling for keyboard users.
