...
## 2026-08-17 - Avoid empty links for JS actions
**Learning:** Implementing interactive elements for JS actions (e.g., clearing data) using `<a href="#">` tags causes unintended scroll to the top of the page when clicked. This disrupts user flow and context, especially in long scrollable lists or dashboards.
**Action:** Instead of empty links, use a semantically correct `<button type="button">` for in-page javascript actions. Add inline CSS resets (`display:inline; background:transparent; border:none; padding:0; font:inherit; cursor:pointer; text-decoration:underline;`) to match the visual style of a link without causing page jumps.
