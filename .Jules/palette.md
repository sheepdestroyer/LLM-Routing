...
## 2026-08-17 - Fix CSS Transforms on Inline Elements
**Learning:** CSS `transform` (like `translateX`) has no visual effect on `inline` elements (e.g. `<span>`, `<a>`) unless `display` is explicitly set to `inline-block` or `block`.
**Action:** Always ensure target elements have `display: inline-block` or `display: block` when applying transform animations for states like `:hover` or `:focus-visible`.
