...
## 2026-08-26 - Accessible Custom Buttons
**Learning:** Using `<a href="#">` tags for interactive JavaScript actions forces an unintended page scroll and creates non-semantic elements. Native `<button>` elements naturally handle keyboard interactions but need their default browser styles reset to visually match anchor links.
**Action:** Replace pseudo-links (`<a href="#">`) used for actions with native `<button type="button">` elements. Reset their styles (`background: transparent; border: none; font: inherit; cursor: pointer;`) and implement `:focus-visible` outlines for keyboard users.
