...
## 2024-08-27 - Native elements over custom interactive spans
**Learning:** Using `<a>` elements for javascript interactions (`href="#"`) causes page jumping and lacks semantic meaning for button-like actions (like clearing form fields).
**Action:** Replace interactive custom `<a>` or `<div>` elements with native `<button type="button">` elements to preserve semantics and default keyboard accessibility.
