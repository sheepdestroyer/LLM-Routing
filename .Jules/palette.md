...
## 2026-08-17 - Avoid link tags for JS actions (clear)
**Learning:** Using `<a href="#">` tags to trigger JavaScript actions (like clearing an annotation) forces an unintended scroll jump to the top of the page because of the anchor tag's default behavior, degrading the UX.
**Action:** Replace `<a href="#">` tags with semantic `<button type="button">` elements. Use a CSS class (like `.clear-btn`) to strip the default button styling (background, border, padding, font inherit) to visually match the original design while adding the required `:focus-visible` state for keyboard accessibility.
