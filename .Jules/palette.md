...
## 2026-08-17 - Keyboard Focus and Hover Parity
**Learning:** CSS animations (like `transform`) applied to focus or hover states on inline elements (like `<a>` or `<span>`) do not visually trigger unless the element is explicitly set to `display: inline-block` or `block`. Additionally, ensuring visual parity between `:hover` and `:focus-visible` states is critical for a consistent experience across mouse and keyboard users.
**Action:** Always check the `display` property of elements receiving transform animations, and ensure that any visual changes applied to `:hover` are identically mirrored in `:focus-visible` to maintain keyboard accessibility.
