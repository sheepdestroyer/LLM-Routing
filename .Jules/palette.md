...
## 2026-10-27 - Replaced interactive anchor with styled button
**Learning:** Using `<a href="#">` with inline `onclick` handlers for actions like clearing data causes the page to unexpectedly jump to the top unless `event.preventDefault()` is used. This disrupts user context, especially in long scrollable lists like the dataset visualizer.
**Action:** Replace interactive links intended for actions with `<button type="button">` and apply CSS resets (`background: transparent; border: none; color: inherit; font-family: inherit; font-size: inherit; cursor: pointer; padding: 0; text-decoration: underline;`) to maintain visual layout while preserving semantic interaction and avoiding page jumps.
