...
## 2026-08-21 - Accessible List Items
**Learning:** Using a custom `div` element with `tabindex="0"` and `role="button"` for interactive list items creates non-semantic markup and requires complex manual event handlers (`onkeydown` checking for Enter/Space), which is an accessibility anti-pattern.
**Action:** Replace interactive list items using custom `div`s with native `<button type="button">` elements. When doing so, ensure you apply CSS rules to reset the default browser button styles (e.g., `display: block; width: 100%; text-align: left; background: transparent; color: inherit; border: none; font-family: inherit;`) to preserve the exact same visual design.
