...
## 2024-05-24 - Avoid Outline on Hover
**Learning:** Applying `outline` (focus rings) on mouse `:hover` states creates a confusing user experience. Focus rings are intended strictly for keyboard navigation (`:focus-visible`); applying them on hover is an accessibility and UX anti-pattern. Additionally, layout-affecting transformations (like `translateX`) on hover can cause jarring "jumps" if not carefully considered for the element's position.
**Action:** When adding hover states, use subtle visual cues like background color changes or opacity shifts. Reserve `outline` and `outline-offset` strictly for `:focus-visible`. Always verify that hover animations don't cause unintended layout shifts.
