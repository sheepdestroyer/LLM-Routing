...
## 2024-08-18 - Master-Detail Empty States & Dynamic Layouts
**Learning:** Dynamic JavaScript single-page interfaces (like the dataset visualizer) often overlook scenarios where arrays are empty (e.g., zero search/filter results) or when no detail item is yet selected. Leaving these areas blank or rendering raw UI structure without content is a poor UX.
**Action:** Always verify that dynamic list/detail UIs implement explicit `.empty-state` blocks to provide helpful guidance or clear feedback when data collections are empty or selections are null. Avoid rendering list containers or empty detail panes if there is nothing to show.
