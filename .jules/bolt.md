...
## 2024-05-18 - Fast Dictionary Copies
**Learning:** Returning `copy.deepcopy()` from an in-memory cache is slow and blocks the event loop.
**Action:** For cached data that callers must not mutate, cache the raw JSON bytes and return `orjson.loads(cached_bytes)` for significantly faster deep copies with less memory overhead.
