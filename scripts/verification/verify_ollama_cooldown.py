...
try:
    from .verification_helpers import get_triage_request_count, load_litellm_key, send_litellm_request
except ImportError:
    from verification_helpers import get_triage_request_count, load_litellm_key, send_litellm_request
...
def main():
    ...
    success2, model_returned2 = send_request(
        "agent-advanced-core", "Design a distributed pub/sub system with Valkey and describe failover states"
    )
    ...
    if count_after_1 > count_init:
        ...
        if success2 and model_returned2 != "llm-routing-ollama" and not model_returned2.startswith("ollama-"):
            print(
                f"✅ SUCCESS: llm-routing-ollama was successfully cooled down and LiteLLM fell back to openrouter-auto (diff={diff}, model={model_returned2})!"
            )
        else:
        ...
        print(
            "❌ FAILURE: First request did not even reach the triage router (check if all free models failed immediately without fallback)."
        )
        sys.exit(1)
