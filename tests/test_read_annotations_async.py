...
async def test_read_annotations_async_initial_read():
    ...
    fake_data_bytes = b'{"annotation1": "data1"}'
    ...
    with patch("os.path.getmtime", return_value=100.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["data"] == fake_data_bytes
...
async def test_read_annotations_async_cache_hit():
    ...
    # Pre-populate cache
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "data": locals().get("fake_data_bytes", fake_data)}
...
async def test_read_annotations_async_cache_invalidation():
    fake_path = "/tmp/annotations.json"
    fake_data_old = b'{"annotation1": "data1"}'
    fake_data_new_bytes = b'{"annotation2": "data2"}'
    ...
    with patch("os.path.getmtime", return_value=200.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["data"] == fake_data_new_bytes

@pytest.mark.asyncio
async def test_read_annotations_async_deepcopy():
    fake_path = "/tmp/annotations.json"
    fake_data = b'{"annotation1": {"nested": "value"}}'
    ...
    with patch("os.path.getmtime", return_value=100.0):
        ...
        import orjson
        assert orjson.loads(router.main._annotations_cache[fake_path]["data"])["annotation1"]["nested"] == "value"
