...
async def test_read_annotations_async_initial_read():
    ...
    with patch("os.path.getmtime", return_value=100.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["content"] == b'{"annotation1": "data1"}'
...
async def test_read_annotations_async_cache_hit():
    ...
    # Pre-populate cache
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "content": b'{"annotation1": "data1"}'}
...
async def test_read_annotations_async_cache_invalidation():
    ...
    with patch("os.path.getmtime", return_value=200.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["content"] == b'{"annotation2": "data2"}'
...
async def test_read_annotations_async_deepcopy():
    ...
    # Pre-populate cache
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "content": b'{"annotation1": {"nested": "value"}}'}
    ...
    with patch("os.path.getmtime", return_value=100.0):
        ...
        assert router.main._annotations_cache[fake_path]["content"] == b'{"annotation1": {"nested": "value"}}'
