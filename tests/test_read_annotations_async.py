...
async def test_read_annotations_async_initial_read():
    ...
    with patch("os.path.getmtime", return_value=100.0) as mock_getmtime, \
        ...
        assert orjson.loads(router.main._annotations_cache[fake_path]["data"]) == fake_data
...
async def test_read_annotations_async_cache_hit():
    ...
    # Pre-populate cache
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "data": json.dumps(fake_data).encode("utf-8")}
...
async def test_read_annotations_async_cache_invalidation():
    ...
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "data": json.dumps(fake_data_old).encode("utf-8")}
    ...
    with patch("os.path.getmtime", return_value=200.0) as mock_getmtime, \
        ...
        assert orjson.loads(router.main._annotations_cache[fake_path]["data"]) == fake_data_new
