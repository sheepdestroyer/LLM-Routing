...
def make_mock_aiofiles_open(content: bytes):
    mock_file = AsyncMock()
...
async def test_read_annotations_async_initial_read():
    ...
    # Mock aiofiles.open
    mock_aiofiles_open = make_mock_aiofiles_open(orjson.dumps(fake_data))
    ...
    with patch("os.path.getmtime", return_value=100.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["raw_bytes"] == orjson.dumps(fake_data)
...
async def test_read_annotations_async_cache_hit():
    ...
    # Pre-populate cache
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "raw_bytes": orjson.dumps(fake_data)}
...
async def test_read_annotations_async_cache_invalidation():
    ...
    router.main._annotations_cache[fake_path] = {"mtime": 100.0, "raw_bytes": orjson.dumps(fake_data_old)}

    mock_aiofiles_open = make_mock_aiofiles_open(orjson.dumps(fake_data_new))
    ...
    with patch("os.path.getmtime", return_value=200.0) as mock_getmtime, \
        ...
        assert router.main._annotations_cache[fake_path]["raw_bytes"] == orjson.dumps(fake_data_new)
...
async def test_read_annotations_async_deepcopy():
    ...
    with patch("os.path.getmtime", return_value=100.0):
        ...
        assert orjson.loads(router.main._annotations_cache[fake_path]["raw_bytes"])["annotation1"]["nested"] == "value"
